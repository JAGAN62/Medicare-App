...
const StatusBadge = ({ itemStatus }) => {
  if (itemStatus === "Completed")
    return <span className={badgeStyles.statusBadge.completed}><CheckCircle className={iconSize.small} /> Completed</span>;
  if (itemStatus === "Confirmed")
    return <span className={badgeStyles.statusBadge.confirmed}><Bell className={iconSize.small} /> Confirmed</span>;
  if (itemStatus === "Pending")
    return <span className={badgeStyles.statusBadge.pending}><Clock className={iconSize.small} /> Pending</span>;
  if (itemStatus === "Canceled")
    return <span className={badgeStyles.statusBadge.canceled}><XCircle className={iconSize.small} /> Canceled</span>;
  return <span className={badgeStyles.statusBadge.default}><CalendarDays className={iconSize.small} /> Rescheduled</span>;
};
...
const AppointmentPage = () => {
  ...
  // ✅ Show toast based on payment_status or service_payment query param
  useEffect(() => {
    const params = new URLSearchParams(location.search);
    const paymentStatus = params.get("payment_status");
    const servicePayment = params.get("service_payment");
    const status = paymentStatus || servicePayment;

    if (status === "Paid") {
      toast.success("🎉 Payment successful! Your appointment is confirmed.", { duration: 5000 });
    } else if (status === "Failed") {
      toast.error("❌ Payment failed. Please try again.", { duration: 5000 });
    } else if (status === "Cancelled") {
      toast("⚠️ Payment was cancelled.", { duration: 4000 });
    }
  }, [location.search]);
  ...
  const loadDoctorAppointments = useCallback(async () => {
    ...
    try {
      ...
      // ✅ Fixed: backend returns { appointment: [...] } (singular), not appointments
      const fetched =
      ...
      console.error("Error calling /api/appointments/me:", err?.response?.data || err.message);

      if (user?.id) {
        try {
          const debugResp = await API.get(`/api/appointments/me?createdBy=${user.id}`, { headers });
          const fetched =
            debugResp?.data?.appointment ??
          ...
          console.error("Debug fallback failed (doctors):", err2?.response?.data || err2.message);
  ...
  const loadServiceAppointments = useCallback(async () => {
    ...
    try {
      ...
      console.error("Error calling /api/service-appointments/me:", err?.response?.data || err.message);
      ...
      if (user?.id) {
        try {
          ...
          console.error("Debug fallback failed (services):", err2?.response?.data || err2.message);
  ...
  function normalizeRescheduled(rt) {
    ...
    if (rt.date && (rt.hour !== undefined || rt.minute !== undefined || rt.ampm)) {
      return { date: rt.date, time: `${rt.hour ?? 0}:${pad(rt.minute ?? 0)} ${rt.ampm ?? ""}` };
    }
  ...
  const appointmentData = useMemo(() => {
    return doctorAppts.map((a) => {
      ...
      const image = doctorObj.imageUrl || doctorObj.image || a.doctorImage?.url || a.doctorImage || "";
      const doctorName =
      ...
      const date = a.date || "";
      let time = a.time || "";
      if (!time && a.hour !== undefined && a.ampm) {
        time = `${a.hour}:${pad(a.minute ?? 0)} ${a.ampm}`;
      }
      const payment = a.payment?.method || "Cash";
      const status = a.status || (a.payment?.status === "Paid" ? "Confirmed" : "Pending");
  ...
  const serviceData = useMemo(() => {
    return serviceAppts.map((s) => {
      ...
      const image = svc.imageUrl || svc.image || s.serviceImage?.url || s.serviceImage || "";
      ...
      const status = s.status || (s.payment?.status === "Paid" ? "Confirmed" : "Pending");
  ...
  return (
    <div className={appointmentPageStyles.pageContainer}>
      ...
      <div className={appointmentPageStyles.maxWidthContainer}>
        ...
        {loadingDoctors && <div className={appointmentPageStyles.loadingText}>Loading Doctors...</div>}
        ...
        <div className={appointmentPageStyles.doctorGrid}>
          {appointmentData.map((item) => (
            <div key={item.id} className={cardStyles.doctorCard}>
              ...
              <p className={cardStyles.dateContainer}><CalendarDays className={iconSize.medium} />{item.date}</p>
              <p className={cardStyles.timeContainer}><Clock className={iconSize.medium} />{item.time}</p>
              ...
              {item.status === "Rescheduled" && item.rescheduledTo && (
                <div className={cardStyles.rescheduledText}>
                  Reschedule to <span className={cardStyles.rescheduledSpan}>{item.rescheduledTo.date} : {item.rescheduledTo.time}</span>
                </div>
              )}
            </div>
          ))}
        ...
        {loadingServices && <div className={appointmentPageStyles.loadingText}>Loading Service Bookings...</div>}
          ...
          {serviceData.map((srv) => (
            <div key={srv.id} className={cardStyles.serviceCard}>
              <div className={cardStyles.serviceImageContainer}>
                <img src={srv.image || "/placeholder-service.png"} alt={srv.name} className={cardStyles.image} loading="lazy" />
              </div>
              <h3 className={cardStyles.serviceName}>{srv.name}</h3>
              <p className={cardStyles.price}>₹{srv.price}</p>
              <p className={cardStyles.serviceDateContainer}><CalendarDays className={iconSize.medium} /> {srv.date}</p>
              <p className={cardStyles.serviceTimeContainer}><Clock className={iconSize.medium} /> {srv.time}</p>
              <div className={cardStyles.badgesContainer}>
              ...
              {srv.status === "Rescheduled" && srv.rescheduledTo && (
                <div className={cardStyles.serviceRescheduledText}>
                  Rescheduled to <span className={cardStyles.rescheduledSpan}>{srv.rescheduledTo.date} : {srv.rescheduledTo.time}</span>
                </div>
              )}
            </div>
          ))}
