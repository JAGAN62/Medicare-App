    ...
    return (
      <select
        ...
        className={`${listPageStyles.statusSelect} ${
          terminal ? listPageStyles.statusSelectDisabled : listPageStyles.statusSelectEnabled
        }`}
        ...
        <div className="flex justify-end">
          <button
            ...
            className={`${listPageStyles.rescheduleButton} ${
              terminal ? listPageStyles.rescheduleButtonDisabled : listPageStyles.rescheduleButtonEnabled
            }`}
          >
            Reschedule
          </button>
...
const ListPage = () => {
  ...
  async function fetchAppointments() {
    ...
    setError(null);
    try {
      const url = `${API_BASE}/api/appointments/doctor/${encodeURIComponent(doctorId)}`;
      ...
      // ✅ Fixed: backend returns { appointment: [...] } (singular), check that first
  ...
  async function rescheduleRemote(id, newDate, newTime24) {
    ...
    setAppointments((prev) =>
      prev.map((p) =>
        p.id === id ? { ...p, date: newDate, time: newTime24, status: "rescheduled" } : p
      )
    );
  ...
  function updateStatus(id, newStatus) { updateStatusRemote(id, newStatus); }
  function updateDateTime(id, newDate, newTime) { rescheduleRemote(id, newDate, newTime); }

  const filtered = useMemo(() => {
    return [...appointments]
      .filter((a) =>
        search ? (a.patient || "").toLowerCase().includes(search.toLowerCase()) : true
      )
                  ...
                  <div className={listPageStyles.cardAvatar}>
                    {a.doctorImage ? (
                      ...
                      <div className={listPageStyles.cardAvatarFallback}>
                        {/* ✅ Fixed typo: (a, doctorName) → a.doctorName */}
                        {(a.doctorName || "D").charAt(0)}
                      </div>
                    )}
                  </div>
                  <div className={listPageStyles.cardContent}>
                    {/* ✅ Fixed: use a.patient (normalized field name) */}
