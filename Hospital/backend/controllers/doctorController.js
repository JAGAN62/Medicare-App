...
// this function will convert time to number of munutes  since midnight;
...
// this function will remove duplictes slot and return the slot filter by time;
...
// convert mognoose map to plain  to text
function normalizeDoctorForClient(raw = {}) {
  const doc = { ...raw };
...
export async function createDoctor(req, res) {
  try {
    ...
    if (!secret) {
      console.warn("JWT Secret key is not defined!");
    ...
    const out = normalizeDoctorForClient(doc.toObject());
    delete out.password;
...
export async function getDoctorsById(req, res) {
  try {
    ...
    return res.json({
      success: true,
      data: normalizeDoctorForClient(doc),
    });
  } catch (err) {
...
export async function updateDoctor(req, res) {
  try {
    ...
    const out = normalizeDoctorForClient(existing.toObject());
    delete out.password;
...
export async function doctorLogin(req,res) {
  try{
    ...
    const token = jwt.sign({
        id:doc.id.toString(),email:doc.email,
        role:"doctor"
