...
export const getAppointments = async (req,res) => {
    ...
    if (search) {
     ...
     return res.json({
        ...
        console.error("getapponintment error",error)
...
export const getAppointmentByPatient = async (req,res) => {
  try {
    ...
    const appointment = await Appointment.find(filter).sort({date : 1,time:1}).lean();
    return res.json({success: true,appointment});
  } catch (error) {
        console.error("getapponintment by patient error",error)
        return res.status(500).json({
            success : false,
            message : 'server error'
        })
...
export const confirmPayment = async (req,res) => {
  try {
    ...
    console.error("payment confiremd eror", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
...
export const getAppointmentsByDoctor = async(req,res)=> {
  try {
    ...
    .populate("doctorId","name specilaiztion owner imageUrl image").lean();
    const total = await Appointment.countDocuments(filter);
    return res.json({
      success : true,
      appointment : items,
    ...
    console.error("getAppointmentsBy doctor",error)
    return res.status(500).json({success : false,message : "server error!"})
  }
}

// to get register user count
export async function getRegisterdUserCount(req,res){
  try {
    const totalUsers = await clerkClient.users.getCount();
    return res.json({success : true,totalUsers})
  } catch (error) {
    console.error("getregister user count error : ",error)
    return res.status(500).json({success : false,message : "server error!"})
  }
}

export default{getAppointmentsByDoctor,getAppointments,getAppointmentByPatient,createAppointment,confirmPayment,updateAppointment,cancelAppointment,getStats,getRegisterdUserCount}