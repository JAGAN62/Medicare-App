    ...
    try {
       const body = req.body || {};
        ...
        if(!clerkUserId) return res.status(401).json({
            success : false,
            message : 'authetication is required  to create a service appointment'
        })
        /* CREATE */
    const {
      serviceId,
...
export const confirmSevicePayment = async(req,res) => {
    try {
...
export const getServiceAppointment = async(req,res) => {
    ...
    return res.json({
        ...
        console.error("getservice payment error : ",error);
        return res.status(500).json({
            success : false,
            message : "server error!"
        })
    ...
    // to get service appointmentbyid
}
...
export const cancelServiceAppointment = async(req,res) => {
    ...
    return res.json({success : true,data : appt})
        ...
        console.error("cancel service appointment:",error)
        return res.status(500).json({
            success : false,
            message : "server error!"
        })
...
export const getServiceAppointmentByPatient = async (req,res) => {
    try {
        ...
        console.error("getServiceAppointmentByPatient error :",error)
        return res.status(500).json({
            success : false,
            message : "server error!"
        })
