...
// so this function takes date-time slot strings and groups them into array
...
export async function getServiceById(req,res){
    try {
        ...
        console.error("getserviceby id error:",err);
        return res.status(500).json({
            success : false,
            message : "server error"
        });
    ...
    try {
        ...
        if(existing.imagePublicId){
            try {
                ...
                console.warn("failed to delete image  form cloudinary:",
                    error?.message || error
                )
            }
        }
        ...
        return res.status(200).json({
            success : true,
            message : "servive deleted!"
        })
    } catch (err) {
        console.error("delete sevice error:",err);
        return res.status(500).json({
            success : false,
            message : "server error"
        });
